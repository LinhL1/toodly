...
/**
 * Remove all completed todos in one write, and prune their ids from group
 * taskIds so no empty group shells are left behind.
 */
export async function clearCompleted() {
  const [todos, groups] = await Promise.all([getTodos(), getGroups()]);
  const removedIds = new Set(todos.filter(t => t.completed).map(t => t.id));
  if (removedIds.size === 0) return;
  const newTodos  = todos.filter(t => !t.completed);
  const newGroups = groups
    .map(g => ({ ...g, taskIds: g.taskIds.filter(id => !removedIds.has(id)) }))
    .filter(g => g.taskIds.length > 0);
  await setMultiple({ [KEYS.TODOS]: newTodos, [KEYS.GROUPS]: newGroups });
}
...
export async function clearCompletedBefore(dateStr) {
  const [all, groups] = await Promise.all([getTodos(), getGroups()]);
  const removedIds = new Set();
  const keep = all.filter(t => {
    if (!t.completed || t.completedAt == null) return true;
    const d  = new Date(t.completedAt);
    const ds = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
    if (ds < dateStr) { removedIds.add(t.id); return false; }
    return true;
  });
  if (removedIds.size === 0) return;
  const newGroups = groups
    .map(g => ({ ...g, taskIds: g.taskIds.filter(id => !removedIds.has(id)) }))
    .filter(g => g.taskIds.length > 0);
  await setMultiple({ [KEYS.TODOS]: keep, [KEYS.GROUPS]: newGroups });
}
...
/** Toggle whether a group is pinned above the Today section. */
export async function toggleGroupPinned(groupId) {
  const groups = await getGroups();
  const group  = groups.find(g => g.id === groupId);
  if (!group) return;
  group.pinnedAboveToday = !group.pinnedAboveToday;
  await setGroups(groups);
}
