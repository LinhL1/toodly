 ...
 *
 * Future days (after today) are drawn as faint outline circles so the
 * full year grid is always visible; the filled portion grows through the year.
 *
...
export function renderGrid(canvas, sessions, { year } = {}) {
  ...
  for (let i = 0; i < dates.length; i++) {
    ...
    if (date > todayStr) {
      // Future day: hollow outline dot, not hoverable
      ctx.beginPath();
      ctx.arc(px, py, ghostDotR, 0, Math.PI * 2);
